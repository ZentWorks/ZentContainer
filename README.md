...
- Scheduled and manual image update checks with persistent update markers, plus controlled container updates with live step-by-step progress, automatic rollback on failure and automatic removal of the temporary rollback container after success
...
ZentContainer can update itself and paired Agents directly from the WebUI. The active Controller/Agent container can also be updated from **Containers -> More -> Check for updates**; ZentContainer automatically routes that exact self container to the safe self-update path. Compose installations are updated through their Compose source when that source is reachable from the Docker host. If Docker Compose was launched by another UI/container and the source paths stored in Docker labels are client-local, ZentContainer safely falls back to exact Docker-inspect recreation when the configured image reference is unchanged (for example `:latest` after pulling a newer digest). Docker-run, Unraid, Portainer and other Docker-created installations use the same exact recreation path. Configuration and network identity are preserved, failures roll back automatically, and paired Agents additionally require the Controller to reconnect over mTLS before their update is committed.
